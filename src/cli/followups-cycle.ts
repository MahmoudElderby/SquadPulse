...
async function main() {
  ...
  if (!opts.noPreview) {
    ...
    slackDelivered = previewPost.slackDelivered;
    lastTs = previewPost.messageTs ?? opts.threadTs;
  }

  if (opts.dryRun && !opts.poll && !opts.commandTranscript && !opts.commandsBridge) {
    emitFollowUpResult({
      status: 'success',
      workflow: 'followups',
      slackDelivered,
  ...
  if (!opts.poll && !opts.commandTranscript && !opts.commandsBridge) {
    emitFollowUpResult({
      status: 'success',
      workflow: 'followups',
      slackDelivered,
